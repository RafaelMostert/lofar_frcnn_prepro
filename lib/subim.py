...
def extract_subim(filename, ra, dec, size, hduid=0, verbose=True, force_132=False):
    if verbose:
    ...
    # imc0 = imc-psize/2
    # skyc0 = lwcs.wcs_pix2world(imc0,0)
    # imc1 = imc+psize/2
    # skyc1 = lwcs.wcs_pix2world(imc1,0)
    x = imc[0][0]
    y = imc[0][1]
    if verbose:
        print("pixelsize:", psize, 'filename', filename, 'size', size, 'header', orighdu[hduid].header['CDELT2'])

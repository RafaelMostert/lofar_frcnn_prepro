...
def get_wise(ra, dec, band, out_dir):
    mission = 'wise'
    dataset = 'allwise'
    table = 'p3am_cdd'
    successful = False
    while not successful:
        try:
            results = Ibe.query_region(coord.SkyCoord(ra, dec, unit=(u.deg, u.deg), frame='icrs'), mission=mission,
    ...
    download_file(url + path, os.path.join(out_dir, outname))
    return outname


def get_first(ra, dec):
    url = "http://archive.stsci.edu/"
    page = requests.get(url + "vlafirst/search.php?RA=%.7f&DEC=%.6f&Radius=30.0&action=Search" % (ra, dec),
                        verify=False)
    print(page.status_code)
...
def get_legacy(ra, dec, size=1000, pixscale=0.454, bands='r', ftype='fits'):
    url = "http://legacysurvey.org/viewer/{}-cutout?ra={}&dec={}&size={}&layer=mzls+bass-dr6&pixscale={}&bands={}".format(
        ftype, ra, dec, size, pixscale, bands)
...
class LofarMaps(object):
    def __init__(self, stay_in_imagedir=False):
        imagedir = os.environ['MOSAICS_PATH_DR2']
        wd = os.getcwd()
        os.chdir(imagedir)
        ...
        for d in g:
            if '.fits' in d:
                ...
                file = os.path.join(d, sys.argv[2] + '.fits')

...
# rms_filename=os.environ['POINTING_NAME']+ '.rms.fits'
# cat_filename=os.environ['POINTING_NAME']+ '.cat.fits'
# field_filename= os.environ['POINTING_NAME']+ '.fits'
...
try:
    ...
    artefact_and_nearby_filtering = True
...
if training_mode:
    # Load value added catalog
    if not 'LOTSS_SOURCE_CATALOGUE' in os.environ:
        new_DR2 = True
        print("We are creating plots for specific new DR2 regions now for which we do",
              " have a comp cat but not a merged cat.")

    vac = pd.read_hdf(os.environ['LOTSS_SOURCE_CATALOGUE'], 'df')
    ...
    compcat_field_names = list(set(compcat['Mosaic_ID']))
    field_folders = [f for n, f in zip(field_names, field_folders) if n in compcat_field_names]
    local_field_folders = [f for n, f in zip(field_names, local_field_folders) if n in
                           compcat_field_names]
    field_names = [n for n in field_names if n in compcat_field_names]
    try:
        convex_path = os.environ["convex"]
        selected_fields = ['P3Hetdex16']
        field_folders = [f for n, f in zip(field_names, field_folders) if n in selected_fields]
        local_field_folders = [f for n, f in zip(field_names, local_field_folders) if n in
                               selected_fields]
        field_names = [n for n in field_names if n in selected_fields]
    except:
        pass
    try:
        test_fields_only = os.environ["test_fields_only"]
        selected_fields = ['P8Hetdex', 'P206+52', 'P16Hetdex13']
        field_folders = [f for n, f in zip(field_names, field_folders) if n in selected_fields]
        local_field_folders = [f for n, f in zip(field_names, local_field_folders) if n in
                               selected_fields]
        field_names = [n for n in field_names if n in selected_fields]
        ...
        pass
    vac_index_name_dict = {n: i for i, n in zip(vac.index.values, vac.Source_Name.values)}
    comp_index_name_dict = {n: i for i, n in zip(compcat.index.values, compcat.Component_Name.values)}

    ##################### DEBUG
    cdict = Counter(compcat[skey].values)
    count = np.array(list(cdict.values()))
    # n_multiples = np.sum(count>1)
    # mean_multiples = np.mean(count[count>1])
    names_multiples = np.array(list(cdict.keys()))[count > 1]
    print(f"Compcat contains {len(compcat)} sources, of which {len(names_multiples)} ",
...
for field_idx, (field_name, field_folder, local_field_folder, field_path, field_cat_path) in enumerate(zip(field_names,
    ...
    """
    if training_mode:
        source_cat = raw_cat[raw_cat.Mosaic_ID == field_name[:8]]
        #source_cat = source_cat[['Source_Name','RA','DEC','Mosaic_ID','LGZ_Size','LGZ_Width','LGZ_PA','Maj','Min','PA','ID_flag','ID_ra','ID_dec']]
    else:
